-- 1. Patch existing rows where collection > penalty_amount (negative balance)
update public.penalty_lines
set penalty_amount = collection
where collection > penalty_amount;

-- 2. Guard in generate_penalty_lines: never let penalty_amount drop below collection on Partial rows.
--    Drop both overloads and recreate.
