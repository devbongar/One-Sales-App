-- Add total_amount_due column to penalty_lines and update RPC to populate it.

alter table public.penalty_lines
  add column if not exists total_amount_due numeric(14,2);

-- Recreate RPC with total_amount_due populated from receivables_database
...
begin
  ...
  loop
    ...
    insert into public.penalty_lines (
      ...
      original_due_date, days_overdue, daily_rate, principal_basis, total_amount_due,
      penalty_amount, payment_status, generated_at
    )
    values (
      r.reservation_id, r.client_id, r.line_id, r.client_name, r.inventory_code,
      r.due_date, v_days, v_rate, r.principal, r.total_amount_due,
