...
export async function fetchSpouseInfo(reservationId: string): Promise<SpouseInfoRecord | null> {
  const { data, error } = await supabase.rpc('get_spouse_info', { p_reservation_id: reservationId });
  if (error) throw error;
  return data as SpouseInfoRecord | null;
}

export async function saveSpouseInfo(payload: SpouseInfoPayload): Promise<void> {
  const { error } = await supabase.rpc('save_spouse_info', {
