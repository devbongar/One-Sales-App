...
function LinesOverlay({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // Compute total paid from actual receipts — accurate across BRF restructuring.
      // Source 1: collection records (posted via collection posting page).
      const collTotal = collData.reduce((sum, c) => sum + Number(c.amount_received), 0);
      // Source 2: legacy lines paid directly (RF, 1st DP via Finance verify — no collection record).
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-2 pb-10">
        {loading ? (
          ...
          <>
            {/* Grouped schedule lines — Overdue → Upcoming → Paid */}
            {(() => {
              ...
              function LineCard({ line }: { line: ReceivableLine }) {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Payment receipts (collection-model: OR#, date, amount per receipt) */}
                    ...
                    {/* Direct payment info (RF, 1st DP verified through Finance, legacy lines) */}
                    {hasDirectPayment && (
            ...
            {(() => {
              ...
              return (
                <div className="space-y-2 pt-2">
                  ...
                  {/* Legacy entries (RF, 1st DP paid via Finance verify) */}
                  {legacyLines.map(l => (
                    <div
                      key={`leg-${l.id}`}
                      className="rounded-3xl bg-white overflow-hidden"
                    ...
                    >
                      <div className="flex items-start justify-between px-4 py-3">
                        <div>
                          ...
                          <p className="text-[11px] text-[#8E8E93] mt-0.5">
                            {[
                              l.posting_date ? fmtDate(l.posting_date) : null,
                              l.mode_of_payment,
                            ].filter(Boolean).join(' · ')}
                          </p>
                        </div>
...
export default function CollectionPostingPage() {
  ...
  async function handleBackfill() {
    ...
    try {
      // Paginated fetch — PostgREST silently caps at 1000 rows without .range()
      const PAGE = 1000;
      const reservations: { reservation_id: string; date_of_reservation_fee: string | null }[] = [];
      let from = 0;
      while (true) {
        const { data, error } = await supabase
          .from('reservations')
          .select('reservation_id, date_of_reservation_fee')
      ...
      for (const r of reservations) {
        // Skip if no RF date — using today would anchor all due dates incorrectly
        if (!r.date_of_reservation_fee) {
          skipped++;
          continue;
        }
        try {
          await generateReceivableLines(r.reservation_id, r.date_of_reservation_fee);
          generated++;
        } catch (e: any) {
          errors.push(`${r.reservation_id}: ${e?.message ?? 'unknown error'}`);
          skipped++;
        }
      }

      if (errors.length > 0) console.error('Backfill errors:', errors);
