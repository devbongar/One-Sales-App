...
export default function ExistingClientPage() {
  ...
  function handleBrokerBirChange(name: string) {
    ...
    setBrokerSalesDirector(rec?.sales_director             ?? '');
  ...
  function loadSellerFromClient(c: ClientRecord) {
    ...
    setSellerDivisionHead(c.sales_division_head ?? '');
    setSellerSalesHead(c.sales_head ?? '');
    setBrokerNetworkAssociate(c.broker_network_associate ?? '');
    setBrokerNetworkOfficer(c.broker_network_officer ?? '');
    setBrokerSalesDirector(c.broker_sales_director ?? '');
  ...
  async function validate() {
    ...
    else if (!e.email && await checkEmailExists(form.email, selectedClient?.id))
  ...
  async function handleSave() {
    ...
    try {
      ...
      const updated: ClientRecord = {
        ...
        sales_division_head:      form.sellerType === 'In House' ? sellerDivisionHead     || null : null,
        sales_head:               form.sellerType === 'In House' ? sellerSalesHead        || null : null,
        ...
        broker_sales_director:    form.sellerType === 'Broker'   ? brokerSalesDirector    || null : null,
            ...
            <div className="px-4 pt-[88px] pb-12 max-w-lg mx-auto w-full">
              ...
              <div className="space-y-4">
                ...
                <DarkSectionCard title="Seller Information" zIndex={2}>
                  ...
                  {/* In House — bidirectional SD ↔ SM ↔ PS */}
                  {form.sellerType === 'In House' && !isMegawideEmployee && (
                    <>
                      <DarkInputRow label="Sales Director" icon={<UserCog size={11} />} error={errors.sellerDirector}>
                        {isUserSD ? (
                          <div className={dReadCls}>{sellerDirector || '—'}</div>
                        ) : (
                          <SearchableCombobox
                            value={sellerDirector}
                            options={sdOptions}
                      ...
                      <DarkInputRow label="Sales Manager" icon={<Users size={11} />} error={errors.sellerManager}>
                        {isUserSM ? (
                          <div className={dReadCls}>{sellerManager || '—'}</div>
                        ) : (
                          <SearchableCombobox
                            value={sellerManager}
                            options={smOptions}
                      ...
                      <DarkInputRow label="Property Specialist" icon={<User size={11} />} error={errors.sellerSpecialist}>
                        {isUserPS ? (
                          <div className={dReadCls}>{sellerSpecialist || '—'}</div>
                        ) : (
                          <SearchableCombobox
                    ...
                    <>
                      ...
                      <DarkInputRow label="Sales Director Head" icon={<UserCog size={11} />}>
                        <SearchableCombobox
                          ...
                          disabled={!editMode || brokerCascadeSource === 'bir' || brokerCascadeSource === 'associate' || brokerCascadeSource === 'officer' || brokerCascadeSource === 'sd'}
                        />
                      </DarkInputRow>
